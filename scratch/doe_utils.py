...
class Encoding_basics: 
    ...
    def __init__(self, mutation_probs_variable_region_dict): 
        ...
        self.shape_single_base_codes = np.shape(self.single_base_codes)
        self.shape_pairwise_base_codes = np.shape(self.pairwise_base_codes)
        # Notice that order in maintained in itertools.product and np.kron---I checked
        # Now I need to collect all of this in codes for every position that actually appear in the SOLD libraries, 
        # Only one matrix will be created, where the first column is 1 for baseline, the second 19 are for base a, 
        # the next 19 are for base b and the rest are for the pairwise bases  
        reference_codes = [] 
        feature_names = [] 
        for j, k in enumerate(self.pos_product): 
            for i, a in enumerate(self.amino_product): 
                if (a[0] in mutation_probs_variable_region_dict[k[0]]) &  (a[1] in mutation_probs_variable_region_dict[k[1]]): 
                    amino_index_pair = self.amino_product.index(''.join(a)) 
                    feature_names.append(str(k[0]) + '-' + str(a[0]) + ':' + str(k[1]) + '-' + str(a[1]))
                    amino_index_1 = AMINO_ACIDS.index(a[0])
                    amino_index_2 = AMINO_ACIDS.index(a[1])
                    # the first 1 is a the constant contribution 
                    code = np.concatenate(([1], self.single_base_codes[amino_index_1, :], \
                                                self.single_base_codes[amino_index_2, :], \
                                                self.pairwise_base_codes[amino_index_pair, :]))
                    reference_codes.append(code) 

        
        self.reference_codes = np.asarray(reference_codes) 
        self.shape_weights  = np.shape(self.reference_codes)
        self.feature_names = np.asarray(feature_names)

        self.single_base_index = np.arange(1, 2*self.shape_single_base_codes[1]+1) 
        self.pairwise_base_index = np.arange(2*self.shape_single_base_codes[1]+1, self.shape_weights[1])
        self.shape_independent_weights = (self.shape_weights[0], len(self.single_base_index))
        self.shape_pairwise_weights = (self.shape_weights[0], len(self.pairwise_base_index))
...
class Sequence_encoder_simplex(Encoding_basics): 
    ...
    def encode_seqs(self, protein_seqs): 
        ...
        for seq in protein_seqs:
            ...
            local_code = np.zeros(self.shape_weights)
            for j, pos in enumerate(self.pos_product): 
                # need to find the index of amino_acid pairs 
                ...
                local_code_J[index, :] = self.reference_codes[index, :]
...
class Create_in_silico_model(Encoding_basics): 
    ...
    """
    def __init__(self, mutation_probs_variable_region_dict, independent_params = None, pairwise_params = None, baseline = 5): 
        """
        Args:
        ...
        # Now I need to use the independent weights to create the ia and jb weights to concatenate to the ia,jb weights and create a single matrix 
        features_split = np.asarray([re.split(r'[-:]', f) for f in self.feature_names]) # all the features names split 
        collector = [] 
        for i,a,j,b in features_split: 
            


        #self.all_weights = np.concatenate([np.ones(self.shape_weights[0])[:, np.newaxis], self.independent_weights_I, \
        #                                  self.independent_weights_J, self.pairwise_weights], axis = 1)
    ...
    def model(self, independent_codes, pairwise_codes): 
        ...
        """
        ans = np.einsum('ijk, jk -> i', codes, self.all_weights) 
        return ans 
    ...
    def plot_weights(self): 
        ...
        """
        # weights 
        I_samples = np.ravel(self.independent_weights_I) 
        GRID_SIZE = 1000
        ...
        plt.title('Distribution of independent weights for pos') 
